...
class NeuralNetwork:
    ...
    def append_layer(self, layer:BaseLayer) -> None:    #'layer' obj of inherited sub-classes
        if layer.trainable:
            layer.initialize(self.weights_initializer, self.bias_initializer) #initializes the current layer with weights/bias
            layer.optimizer = copy.deepcopy(self.optimizer)    #sets the layer's attribute with optimization_class for eligible layers

        self.layers.append(layer)     #creates self.layers with trainable as well as non-trainable layers(FullConn->ReLU->FullConn->Softmax)
